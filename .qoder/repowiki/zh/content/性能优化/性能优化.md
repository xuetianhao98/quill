...
### 瞬时事件缓冲（TransitEventBuffer）
- 环形缓冲设计
  - 使用2次幂容量与掩码运算实现高效的环形索引，避免昂贵的取模运算。
  - 支持动态扩容，当缓冲区满时按2倍增长策略扩展容量。
- 扩展与收缩
  - 扩容时重新分配内存并移动现有元素，保持事件顺序不变。
  - 支持在空缓冲区时收缩回初始容量，避免内存浪费。
- 性能优化
  - 使用掩码运算实现环形索引，减少分支预测失败。
  - 批量移动元素时使用移动赋值，减少拷贝开销。
...
**更新说明**
基于最新的代码变更，MathUtilities.h 现已正确包含 Common.h，消除了 clangd 错误。这一更新改善了数学工具函数与公共头文件之间的依赖关系，确保了编译时的正确性和工具链支持。

**更新内容**
- MathUtilities.h 现在直接包含 Common.h，提供了必要的缓存行常量和断言宏定义
- TransitEventBuffer.h 也正确包含 MathUtilities.h，用于容量计算和掩码运算
- 所有使用 next_power_of_two 函数的组件都受益于这一改进的依赖关系

**影响范围**
- BoundedSPSCQueue.h：使用 next_power_of_two 进行容量对齐
- TransitEventBuffer.h：使用 next_power_of_two 进行动态扩容
- BackendWorker.h：间接通过其他组件使用数学工具函数
- 所有单元测试：MathUtilitiesTest.cpp 验证数学工具函数的正确性
